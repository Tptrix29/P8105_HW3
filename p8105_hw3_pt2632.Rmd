...
In 2002, states including `r cleaned_brfss |> filter(year == 2002) |> group_by(locationabbr) |> summarize(n_location = n()) |> filter(n_location >= 7)` were observed at 7 or more locations.

In 2010, states including `r cleaned_brfss |> filter(year == 2010) |> group_by(locationabbr) |> summarize(n_location = n()) |> filter(n_location >= 7)` were observed at 7 or more locations.
